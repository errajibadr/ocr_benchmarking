...
A comprehensive tool for testing latency and throughput of VLLM OCR endpoints
with concurrent requests, detailed metrics, and performance analysis.
"""
...
class OCRResult(BaseModel):
    ...
    text: str = Field(description="The extracted text from the image with proper formatting")


class AsyncVLLMOCR:
    """Async VLLM OCR client for concurrent requests."""
    ...
    def estimate_tokens(self, text: str, image_path: str) -> tuple[int, int]:
        ...
        system_message = "You are An excellent OCR assistant. Extract All Text from the image. Format it as markdown with proper structure and layout"
        text_tokens = len(encoding.encode(system_message))

        # Estimate image tokens (typical vision models use ~85-170 tokens per image)
        # This is a rough approximation - actual token count depends on image size and model
            ...
            try:
                ...
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        # If no token usage provided, estimate tokens
...
class BenchmarkRunner:
    ...
    async def run_fixed_requests(self, image_path: str) -> BenchmarkMetrics:
        ...
        async with AsyncVLLMOCR(self.config) as ocr_client:
            ...
            tasks = [limited_request() for _ in range(self.config.total_requests)]
    ...
    def print_results(self, metrics: BenchmarkMetrics) -> None:
        ...
        """
        print("\n" + "=" * 60)
        print("VLLM OCR BENCHMARK RESULTS")
        print("=" * 60)
...
async def main():
    ...
    try:
        ...
        runner.save_results(metrics, "benchmark_results.json")
    except Exception as e:
